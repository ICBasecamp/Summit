...
const AllMessagesDisplay: React.FC<{ ticker: string }> = ({ ticker }) => {
  ...
  useEffect(() => {
    ...
    if (earningsReportMessage) {
      const statisticalAnalysisData = earningsReportMessage.match(/'name': 'eps_trend_df', 'random_forest_importances': (\{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}), 'pca_components': (\{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\})/);
  ...
  useEffect(() => {
    const earningsReportMessage = messages?.find(message => message.includes("earnings:"));
    if (earningsReportMessage) {
      const earningsEstimationData = earningsReportMessage.match(/\*\*EPS Trend Estimate\*\*([\s\S]*?)\*\*/);
              ...
              <div className="grow w-2/5">
                <div className="flex flex-col gap-8">
                  <div className="flex flex-col rounded-xl bg-zinc-800 p-6 gap-2">
                    <h2 className="text-lg font-medium self-center">EPS Trend Analysis</h2>
                    {earningsEstimation ? (
