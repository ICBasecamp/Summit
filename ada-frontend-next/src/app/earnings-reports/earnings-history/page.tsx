...
const AllMessagesDisplay: React.FC<{ ticker: string }> = ({ ticker }) => {
  ...
  useEffect(() => {
    ...
    if (earningsReportMessage) {
      const revenueDataMatch = earningsReportMessage.match(/'name': 'earnings_history_df', 'random_forest_importances': (\{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}), 'pca_components': (\{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\})/);
  ...
  useEffect(() => {
    const earningsReportMessage = messages?.find(message => message.includes("earnings:"));
    if (earningsReportMessage) {
      const earningsEstimationData = earningsReportMessage.match(/\*\*Earnings History Estimate\*\*([\s\S]*?)\*\*/);
              ...
              <div className="grow w-2/5">
                <div className="flex flex-col gap-8">
                  <div className="flex flex-col rounded-xl bg-zinc-800 p-6 gap-2">
                    <h2 className="text-lg font-medium self-center">Earnings History Analysis</h2>
                    {earningsEstimation ? (
